...
def index_repo():
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        raise Exception(
            "OPENAI_API_KEY environment variable is not set. Please export it to generate embeddings."
        )
    ...
    for dirpath, dirnames, filenames in os.walk(root_dir):
        ...
        for file in filenames:
            ...
            if not file.endswith(
                (".py", ".md", ".json", ".txt", ".js", ".ts", ".html", ".css", ".toml")
            ):
                continue
            ...
            for i, chunk in enumerate(chunks):
                ...
                response = embedding(model="text-embedding-ada-002", input=[chunk])
