...
class SwarmManager:
    ...
    def process_intents(self):
        ...
        try:
            intents = self.session.query(Intent).filter(Intent.status == 'PENDING').all()
            if not intents:
                console.print("[bold yellow]No PENDING intents found in the Vector Metastore.[/bold yellow]")
                return
            ...
            for intent in intents:
                ...
                try:
                    ...
                    response, applied_files = self.builder.execute_intent(intent.description, hot_context=hot_context)
                    ...
                    if applied_files:
                        ...
                        console.print("[bold yellow]No code files generated. Intent resolved as discussion/clarification.[/bold yellow]")
                        intent.status = 'RESOLVED'  
                        self.session.commit()
                except Exception as e:
                    console.print(f"[bold red]Exception during orchestration of intent {intent.id}:[/bold red] {str(e)}")
                    self.session.rollback()
            
            # Final sync if the swarm finishes and queue ends
            if hot_context:
                console.print("[bold cyan]Swarm loop finalizing. Flushing remaining Hot Context to LanceDB...[/bold cyan]")
                reindex_files(list(hot_context.keys()))
                hot_context.clear()
